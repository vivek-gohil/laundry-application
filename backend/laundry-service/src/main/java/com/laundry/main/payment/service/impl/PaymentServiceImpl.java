...
public class PaymentServiceImpl implements PaymentService {
  ...
  public PaymentResponse createPayment(PaymentRequest request) {
    ...
    BigDecimal totalPaid = paymentRepository.getTotalPaidAmount(order.getOrderId());

    BigDecimal orderAmount = order.getTotalAmount();

    BigDecimal newTotalPaid = totalPaid.add(request.getPaymentAmount());
    ...
    payment.setPaymentReference(ReferenceNumberGenerator.generate(ReferencePrefix.PAYMENT));

    payment.setPaymentDate(LocalDateTime.now());

    payment.setPaymentStatus(determinePaymentStatus(orderAmount, newTotalPaid));
  ...
  private void validatePayment(BigDecimal orderAmount, BigDecimal newTotalPaid) {

    if (newTotalPaid.compareTo(orderAmount) > 0) {

      throw new BadRequestException("Payment amount exceeds order amount.");
    }
  }

  private PaymentStatus determinePaymentStatus(BigDecimal orderAmount, BigDecimal totalPaid) {
