...
public class ServiceMasterServiceImpl implements ServiceMasterService {
  ...
  public List<ServiceMasterResponse> getAllServices() {
    return serviceMasterRepository.findAll().stream().map(serviceMasterMapper::toResponse).toList();
  }
  ...
  private void validateUniqueService(ServiceMasterRequest request, Long serviceId) {
    boolean exists =
        serviceId == null
            ...
            : serviceMasterRepository.existsByServiceNameAndServiceIdNot(
                request.getServiceName(), serviceId);
    if (exists) {
      throw new DuplicateResourceException(
          "Service already exists with name: " + request.getServiceName());
    }
  }
}
