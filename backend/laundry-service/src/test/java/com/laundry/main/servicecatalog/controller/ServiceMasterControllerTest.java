...
class ServiceMasterControllerTest {
    ...
    @InjectMocks
    private ServiceMasterController serviceMasterController;
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(serviceMasterController)
    ...
    void getServiceByIdReturnsService() throws Exception {
        ...
        mockMvc.perform(get("/api/services/{serviceId}", 1L))
    ...
    void updateServiceReturnsUpdatedService() throws Exception {
        ...
        when(serviceMasterService.updateService(eq(1L), any(ServiceMasterRequest.class))).thenReturn(response);

        mockMvc.perform(put("/api/services/{serviceId}", 1L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void deleteServiceReturnsSuccessResponse() throws Exception {
        ...
        mockMvc.perform(delete("/api/services/{serviceId}", 1L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void getServiceByIdReturnsNotFoundForMissingService() throws Exception {
        when(serviceMasterService.getServiceById(99L)).thenThrow(new ResourceNotFoundException("Service not found"));

        mockMvc.perform(get("/api/services/{serviceId}", 99L))
